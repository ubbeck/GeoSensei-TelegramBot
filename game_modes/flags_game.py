...
def get_candidates() -> list:
    ...
    for i in index:
        candidates.append(countries[i]['country'])
    return candidates
...
def check_answer(message, bot: telebot, user: dict, country: str) -> None:
    ...
    if 'timer' not in user:
        ...
        user['hits'] = 0
        user['playedInThisRound'].clear()
        bot.send_message(message.chat.id, "Exiting Flags Mode", reply_markup = ReplyKeyboardRemove())
        time.sleep(1)
...
def next_question(message, bot: telebot, user: dict) -> None:
    ...
    while(country['country'] in candidates):
    ...
    user['timer'].start()
    answer = bot.send_message(message.chat.id, (f"{country['flag'].encode().decode('unicode_escape')}"),reply_markup=markup)
    bot.register_next_step_handler(answer, check_answer, bot, user, country['country'])
